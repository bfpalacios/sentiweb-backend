...
public class EncuestaController {
	...
	//reporte pastel 
	//probado
	@GetMapping("/encuesta/estado/{flagSenti}/{idEnc}/{idPreg}")
...
where id_enc=2
    */
    ...
    public ResponseEntity<List<Area>> getEncuestaByPendiente(@PathVariable(value = "idArea") Long idArea,
    		@PathVariable(value = "idEnc") Long idEnc
    ){
    	List<Area> list = encuestaService.getEncuestaByPendiente(idArea, idEnc);
    	return new ResponseEntity<List<Area>>(list, HttpStatus.OK);
    }
    
    @GetMapping("/encuesta/respuondieron/{idArea}/{idEnc}")
    public ResponseEntity<List<Encuesta>> getEncuestaByRespondieron(@PathVariable(value = "idArea") Long idArea,
    		@PathVariable(value = "idEnc") Long idEnc
    ){
